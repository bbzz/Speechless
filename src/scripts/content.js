(async function () {
  ...
  const appendPostToBody = function (post) {
    ...
    // Count could be zero
    if (
      !isNullOrUndefined(post.reposts_count) &&
    ...
    const title = post.topic_struct?.length
      ? post.topic_struct.map((item) => item.topic_title)
      : post.text;

    metaHTML += `<div class="meta">
                    <div class="meta-info">
                        <span class="date">${getDate(post.created_at)}</span>`;
    metaHTML += `<div><a href="https://weibo.com/${id}/${
      post.id
    }">${title.slice(0, 10)}...</a></div>`;
    ...
    if (post.pic_infos) {
      ...
      for (let key in post.pic_infos) {
        mediaHTML += `<img class="image" src="${post.pic_infos[key].largest.url}" />`;
      }
      mediaHTML += '</div>';
    }
